/**
 * Generated by orval v7.21.0 🍺
 * Do not edit manually.
 * Title
 * Description
 * OpenAPI spec version: 1.0
 */
import type { PrivateCreativeTaskSubmissionItemDto } from './privateCreativeTaskSubmissionItemDto';
import type { UpdatePrivatePublicationUrlResponseDtoStatus } from './updatePrivatePublicationUrlResponseDtoStatus';

export interface UpdatePrivatePublicationUrlResponseDto {
  /** Unique identifier */
  id: string;
  /** Timestamp of when the record was created */
  createdAt: string;
  /** Timestamp of the last update of the record */
  updatedAt: string;
  /** Под-ответы (публикации): у каждой свой набор материалов и свой креатив в ОРД */
  items: PrivateCreativeTaskSubmissionItemDto[];
  /**
   * Комментарий от амбассадора
   * @nullable
   */
  comment?: string | null;
  /** Статус проверки ответа */
  status: UpdatePrivatePublicationUrlResponseDtoStatus;
  /**
   * Комментарий от проверяющего
   * @nullable
   */
  reviewComment?: string | null;
  /** ID приватного задания */
  privateTaskId: string;
  /** ID амбассадора */
  ambassadorId: string;
}
